(ns marketentry.store
  "SSoT for the Burundi (BDI) market-entry compliance actor, behind a
...
(defn demo-data
  ...
  `:gre-a-gre-base-value` / `:gre-a-gre-complementaire-value` /
  `:gre-a-gre-claim?` are ground truth for the
  `gre-a-gre-complementaire-threshold-mismatch` flagship check (Code des
  Marchés Publics Art. 37 al. 8 / Art. 40 twenty-percent-of-base-contract
  ceiling); `:requires-nif?` / `:nif-verified?` are ground truth for the
  conditional NIF check. Amounts are in FBU (Burundian Franc)."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kirundo Prestige Services SARL" :portal "armp.gov.bi"
             :base-fee 15000000 :monthly-rate 800000 :monitoring-months 12
             :claimed-fee 24600000.0
             ...
             :jurisdiction "BDI" :status :intake}
    "eng-2" {:id "eng-2" :operator "Ruzizi Logistics Ltd" :portal "armp.gov.bi"
             :base-fee 15000000 :monthly-rate 800000 :monitoring-months 12
             :claimed-fee 24600000.0
             :gre-a-gre-base-value 50000000 :gre-a-gre-complementaire-value 8000000
             :gre-a-gre-claim? true
    ...
    "eng-3" {:id "eng-3" :operator "Gitega Batir Travaux SA" :portal "armp.gov.bi"
             :base-fee 15000000 :monthly-rate 800000 :monitoring-months 12
             :claimed-fee 29900000.0
             :gre-a-gre-base-value 50000000 :gre-a-gre-complementaire-value 8000000
             :gre-a-gre-claim? true
             :requires-nif? true :nif-verified? true
             :drafted? false :submitted? false
             :jurisdiction "BDI" :status :intake}
    "eng-4" {:id "eng-4" :operator "Bujumbura Numérique SAS" :portal "armp.gov.bi"
             :base-fee 15000000 :monthly-rate 800000 :monitoring-months 12
             :claimed-fee 24600000.0
             ...
             :drafted? false :submitted? false
             :jurisdiction "BDI" :status :intake}
    "eng-5" {:id "eng-5" :operator "Muyinga Grands Travaux SPRL" :portal "armp.gov.bi"
             :base-fee 15000000 :monthly-rate 800000 :monitoring-months 12
             :claimed-fee 24600000.0
             :gre-a-gre-base-value 50000000 :gre-a-gre-complementaire-value 8000000
             :gre-a-gre-claim? true
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               gre-a-gre-base-value gre-a-gre-complementaire-value gre-a-gre-claim?
  ...
  (cond-> {:engagement/id id}
    ...
    gre-a-gre-base-value                  (assoc :engagement/gre-a-gre-base-value gre-a-gre-base-value)
    gre-a-gre-complementaire-value        (assoc :engagement/gre-a-gre-complementaire-value gre-a-gre-complementaire-value)
    (some? gre-a-gre-claim?)              (assoc :engagement/gre-a-gre-claim? gre-a-gre-claim?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/gre-a-gre-base-value :engagement/gre-a-gre-complementaire-value :engagement/gre-a-gre-claim?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :gre-a-gre-base-value (:engagement/gre-a-gre-base-value m)
     :gre-a-gre-complementaire-value (:engagement/gre-a-gre-complementaire-value m)
     :gre-a-gre-claim? (boolean (:engagement/gre-a-gre-claim? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-bdi"})

  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kirundo Prestige Services SARL"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kirundo Prestige Services SARL" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest gre-a-gre-complementaire-threshold-mismatch-is-held-and-unoverridable
  (testing "claimed gre-a-gre eligibility doesn't match the independently recomputed Art. 37/40 threshold -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:gre-a-gre-complementaire-threshold-mismatch} (-> (store/ledger db) last :basis)))
